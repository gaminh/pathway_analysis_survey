...
PADOGComb <- bilevelAnalysisGeneset(gslist, gs.names, splitSize =
                                      ncol(dataList[[1]]), dataList, groupList, enrichment = "PADOG", NI=200)
...
pdf(paste0("ROC_PADOG_", dataset,"_", targetSymbol,".pdf"), width = 5, height = 5)
plot(rocobj,legacy.axes=TRUE, main= "ROC curve of PADOG", percent=TRUE, col="red", print.auc=T )
dev.off()
...
save(PADOGComb, temp, rocobj, TP, FP, FN, TN, sensitivity, accuracy, specificity,  file = paste0("padog",dataset, "_",targetSymbol, ".RData"))
...
temp = cbind(temp,label)
colnames(temp) = c("Pathway", "pvalue", "label")
temp = na.omit(temp)
# temp = as.data.frame(temp)
PADOGtest = temp

temp = as.matrix(PADOGtest)
label <- rep(NA, nrow(temp))
names(label) <- rownames(temp)
label[names(posPredict)] = 1
label[names(negPredict)] = 0
...
save(PADOGComb, temp, rocobj, TP, FP, FN, TN, sensitivity, accuracy, specificity,  file = paste0("padog",dataset, ".RData"))

...
resNullKS=data.frame(row.names=pathName, Name=pathName,M=rep(NA,length(pathName)),std=rep(NA,length(pathName)))
for (i in 1:noLoop) {
  if (length(resNull[[i]][,"pvalue"])==0) {resNullKS[,paste("p_",i,sep="")]=NA}
  else {resNullKS[as.vector(resNull[[i]][,"Name"]),paste("p_",i,sep="")]=resNull[[i]][,"pvalue"]}
}
resNullKS$M=apply(resNullKS[4:(noLoop+3)],1,FUN=mean,na.rm=T)
resNullKS$std=apply(resNullKS[4:(noLoop+3)],1,FUN=sd,na.rm=T)

file = paste0("KS",nc,"vs",nd,"_400_",loopNo,".RData",sep="") 
save(resNullKS,l,file=file)


load("/Users/GaMinh/Dropbox/WSU/Papers/PathwayReview/KS15vs15_400_2000(2nd run).RData")
PDFPath = "/Users/GaMinh/Dropbox/WSU/Papers/PathwayReview/generateNullKS(2nd run).pdf"
pathName <- resNullKS[,"Name"]
pdf(file=PDFPath)

for (i in seq(length(pathName)))  {
  print(i)
  hist(as.numeric(resNullKS[i,-c(1,2,3)]), breaks = 50, main = resNullKS[i,]$Name)
}
dev.off()

PDFPath = "/Users/GaMinh/Dropbox/WSU/Papers/PathwayReview/generateNullKS_color(2nd run).pdf"

pdf(file=PDFPath)

for (i in seq(length(pathName)))  {
  print(i)
  hist(as.numeric(resNullKS[i,-c(1,2,3)]), breaks = 50, main = resNullKS[i,]$Name, col = "blue")
}
dev.off()

PDFPath = "/Users/GaMinh/Dropbox/WSU/Papers/PathwayReview/UnderTheNull/generateNullKS_20bins.pdf"
pathName <- resNullKS[,"Name"]
pdf(file=PDFPath)

for (i in seq(length(pathName)))  {
  print(i)
  hist(as.numeric(resNullKS[i,-c(1,2,3)]), breaks = 20, main = resNullKS[i,]$Name)
}
dev.off()

PDFPath = "/Users/GaMinh/Dropbox/WSU/Papers/PathwayReview/UnderTheNull/generateNullKS_20bins_color.pdf"

pdf(file=PDFPath)

for (i in seq(length(pathName)))  {
  print(i)
  hist(as.numeric(resNullKS[i,-c(1,2,3)]), breaks = 20, main = resNullKS[i,]$Name, col = "blue")
}
dev.off()

PDFPath = "/Users/GaMinh/Dropbox/WSU/Papers/PathwayReview/UnderTheNull/generateNullKS_density.pdf"

pdf(file=PDFPath)

for (i in seq(length(pathName)))  {
  print(i)
  x <- density(na.omit(as.numeric(resNullKS[i,-c(1,2,3)])))
  plot(x, main = resNullKS[i,]$Name) 
